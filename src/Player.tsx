...
import { RapierRigidBody, RigidBody, useRapier, Vector3Object } from '@react-three/rapier';
...
export default function Player() {
  ...
  const jump = () => {
    const origin = body.current?.translation() as Vector3Object;
    origin.y -= 0.31;
    const direction = { x: 0, y: -1, z: 0 };
    const ray = new rapier.Ray(origin, direction);
    const hit = rapierWorld.castRay(ray, 10, true);

    if (hit && hit.toi < 0.15) body.current?.applyImpulse({ x: 0, y: 0.5, z: 0 }, true);
  };
  ...
  useFrame((state, delta) => {
    ...
    // Camera
    const bodyPosition = body.current?.translation() as Vector3Object;
