...
interface LevelProps {
  seed?: number;
  count?: number;
  types?: Array<(props: BlockProps) => JSX.Element>;
}
...
export function BlockEnd({ position = [0, 0, 0] }: BlockProps) {
  ...
  return (
    <group position={position}>
      <Text font="bebas-neue-v9-latin-regular.woff" scale={1} position={[0.2, 2.25, 2]}>
        FINISH
        <meshBasicMaterial toneMapped={false} />
      </Text>
      <mesh
        ...
        receiveShadow
      />
      <RigidBody type="fixed" colliders="hull" position={[0, 0.8, 0]}>
  ...
  return (
    <>
      ...
      {blocks.map((Block, idx) => (
        <Block key={`block_${idx}`} position={[0, 0, -4 * (idx + 1)]} />
      ))}
